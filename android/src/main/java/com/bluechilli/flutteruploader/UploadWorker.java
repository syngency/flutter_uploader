...
public class UploadWorker extends ListenableWorker implements CountProgressListener
{
    ...
    public Result doWorkInternal()
    {
        ...
        try
        {
            ...
            {
                return Result.failure(createOutputErrorData(UploadStatus.FAILED, DEFAULT_ERROR_STATUS_CODE, "invalid_url", "url is not a valid url", null));
            }
            ...
            hasJsonResponse = responseContentType != null && responseContentType.contains("json") && body != null;
            ...
            {
                return Result.failure(createOutputErrorData(UploadStatus.FAILED, statusCode, "upload_error", responseString, null));
            }

            Data.Builder builder = new Data.Builder().putString(EXTRA_ID, getId().toString()).putInt(EXTRA_STATUS, UploadStatus.COMPLETE).putInt(EXTRA_STATUS_CODE, statusCode).putString(EXTRA_HEADERS, responseHeaders);
            ...
            catch (IllegalStateException e)
            {
                ...
                Log.d(TAG, "IllegalStateException while building a outputData object. Replace response with" + " on-disk reference.");
    ...
    private void startEngine()
    {
        ...
        if (callbackHandle != -1L && engine == null)
        {
            ...
            engine.getDartExecutor().executeDartCallback(new DartExecutor.DartCallback(context.getAssets(), dartBundlePath, callbackInfo));
        }
    }
    ...
    private Result handleException(Context context, Exception ex, String code)
    {
        ...
        return Result.failure(createOutputErrorData(finalStatus, 500, finalCode, ex.toString(), getStacktraceAsStringList(ex.getStackTrace())));
    }
    ...
    private MultipartBody.Builder prepareRequest(Map<String, String> parameters, String boundary)
    {
        ...
        MultipartBody.Builder requestBodyBuilder = boundary != null && !boundary.isEmpty() ? new MultipartBody.Builder(boundary) : new MultipartBody.Builder();
    ...
    {
        setProgressAsync(new Data.Builder().putInt("status", status).putInt("progress", progress).build());
    }

    private Data createOutputErrorData(int status, int statusCode, String code, String message, String[] details)
    {
        return new Data.Builder().putInt(UploadWorker.EXTRA_STATUS_CODE, statusCode).putInt(UploadWorker.EXTRA_STATUS, status).putString(UploadWorker.EXTRA_ERROR_CODE, code).putString(UploadWorker.EXTRA_ERROR_MESSAGE, message).putStringArray(UploadWorker.EXTRA_ERROR_DETAILS, details).build();
    }
...
class SyngencyInterceptor implements Interceptor
{
    @NonNull
    @Override
